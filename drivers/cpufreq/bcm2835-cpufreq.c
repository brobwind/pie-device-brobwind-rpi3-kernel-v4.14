...
{
	if (!rpi_firmware_get(NULL)) {
		print_err("Firmware is not available, deferring probe\n");
		return -EPROBE_DEFER;
	}
...
static int bcm2835_cpufreq_remove(struct platform_device *pdev)
{
...
static const struct of_device_id bcm2835_cpufreq_match[] = {
	{ .compatible = "brcm,bcm2835-cpufreq" },
	{ }
};
MODULE_DEVICE_TABLE(of, bcm2835_cpufreq_match);


static struct platform_driver bcm2835_cpufreq_platdrv = {
	.driver = {
		.name           = "bcm2835-cpufreq",
		.owner          = THIS_MODULE,
		.of_match_table	= bcm2835_cpufreq_match,
	},
	.probe		= bcm2835_cpufreq_probe,
	.remove		= bcm2835_cpufreq_remove,
};
module_platform_driver(bcm2835_cpufreq_platdrv);

MODULE_AUTHOR("Dorian Peake and Dom Cobley");
MODULE_ALIAS("platform:bcm2835-cpufreq");
